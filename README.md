...
- 智能 AI 交互：AI能够直接搜索媒体条目，并告诉你最近上新了什么。支持自然语言搜索和指令搜索，可按观看状态（已看/未看）过滤搜索结果
...
- 指令搜索：`/emby search <关键词> [全部/已看/未看] [搜索数量]`。
- 观看状态过滤：`已看` 仅返回该用户已看完的条目，`未看` 仅返回未看过的条目，不填写时默认 `全部`。
- 自然语言：直接说出类似“库里面xxxxxxx”的话，也可以问“我看过哪些xxx”，AI 会通过搜索工具自动带上观看状态过滤。
