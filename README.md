...
- 智能 AI 交互：AI能够直接搜索媒体条目，并告诉你最近上新了什么。支持自然语言搜索和指令搜索
...
- `/emby search/latest/detail`分别表示：搜索、查看最近上新、查看媒体元数据。需先完成 UID 绑定后才能使用。
...
- 指令操作：`/emby latest`、`/emby latest <展示数量>`、`/emby latest <类型>`、`/emby latest <类型> <展示数量>`，查看库中最近上新的媒体条目。
- 支持类型：`全部`、`电影`、`电视剧`、`音乐`、`家庭视频`、`照片`。不填写类型时默认使用`全部`，且会显式查询媒体类型 `Movie`、`Episode`、`Audio`、`Video`、`Photo`。
- 自然语言：直接说出“最近库里有什么上新的”。
